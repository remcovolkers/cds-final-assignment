...
- **Toelichting:**
  ...
  datastructuren en/of zoekalgoritmen. Zoeken is momenteel mogelijk voor de volgende
  datastructuren:
...
- **Beschrijving van de functionaliteit:**
  Op basis van een ingegeven stationscode van de gebruiker wordt het gevonden station teruggegeven.
    ...
    - **Sorteer Algoritmen:**
        - **Insertion Sort:** Implementatie in [InsertionSort](src/sortalgorithms/InsertionSort.java). Sorteert een
          lijst
          door elk element te vergelijken met de voorgaande elementen en het op de juiste positie in te voegen.
        - **Quick Sort:** Implementatie in [QuickSort](src/sortalgorithms/QuickSort.java). Dit is een efficiënt
          sorteeralgoritme dat gebruik maakt van een divide-and-conquer strategie om elementen rond een 'pivot' te
          ordenen.

### **Testen:**

#### Testen voor de `BinarySearch` Implementatie in [BinarySearchTest](test/searchalgorithms/BinarySearchTest.java)
...
2. **Test zoeken van onbekend element:**
    ...
    - **Testmethode:** `testSearchNotFound()`
...
3. **Test zoeken in lege lijst:**
    ...
    - **Testmethode:** `testSearchEmptyList()`
...
4. **Test zoeken op uitersten:**
    ...
    - **Testmethode:** `testSearchAtBoundaries()`
...
#### Testen voor de `LinearSearch` Implementatie in [LineairSearchTest](test/searchalgorithms/LinearSearchTest.java)
...
2. **Test Zoeken van Niet-Bestaand Element:**
...
#### Big-O Grafieken van Sorteer Algoritmen
...
Hieronder zijn de grafieken die de tijdcomplexiteit van Insertion Sort en Quick Sort illustreren:
...
1. **Linear Search:**
    - **Complexiteit:** O(n) - De tijd om een element te vinden is proportioneel aan de grootte van de dataset. In het
      slechtste geval moet elk element in de lijst gecontroleerd worden.

2. **Binary Search:**
    - **Complexiteit:** O(log n) - De zoekruimte wordt bij elke stap gehalveerd, wat leidt tot een logaritmische
      zoekduur. Binary search vereist een gesorteerde lijst.

## Rubriek 2: Linked List
...
- **Toelichting en Onderbouwing:**
  ...
  linkedlist en heb hem gebruikt om een stations2 te vullen in [ApplicationData](src/core/ApplicationData.java) → zie
...
#### **Testen:** [RemcoList test](test/datastructures/RemcoListTest.java)

De RemcoList wordt in de test gevuld door [mockstations.csv](test/resources/mockstations.csv)

1. **Test lege RemcoList (`testIsEmpty`):**
    - Controleert of de `isEmpty` methode correct aangeeft of de lijst leeg is.
    - Verwacht resultaat: `false` voor een niet-lege lijst en `true` voor een nieuwe, lege lijst.
...
3. **Test toevoegen van element (`testAdd`):**
    ...
    - Verwacht resultaat: grootte van de lijst is 4 na het toevoegen van een nieuw station.
...
5. **Test verwijderen van element (`testRemove`):**
    ...
    - Verwacht resultaat: Grootte van de lijst is 2 en `contains` retourneert `false` voor het verwijderde element.
...
- **Beschrijving van de Functionaliteit:** De RemcoHashmap functioneert 'gewoon' als een java hashmap zoals ik hem ken.
  ...
  toevoegen met een Key en een Value. En heb hierdoor ook een private class
...
#### **Testen:** [RemcoHashmap test](test/datastructures/RemcoHashMapTest.java)

De RemcoHashmap wordt in de test gevuld door [mockstations.csv](test/resources/mockstations.csv)

1. **Test put en get (`testPutAndGet`):**
...
2. **Test grootte van de RemcoHashmap (`testSize`):**
...
3. **Test collisions (`testCollisionHandling`):**
    - Controleert hoe RemcoHashmap botsingen afhandelt, bijvoorbeeld wanneer twee verschillende keys dezelfde
      hashcode hebben.
    - Verwacht resultaat: De hashmap slaagt erin om beide stations met unieke sleutels correct op te slaan en op te
      halen, ondanks dezelfde hashcode.

4. **Test replace value (`testReplaceValue`):**
    - Test of het vervangen van een waarde voor een bestaande sleutel correct werkt met de `put` methode.
    - Verwacht resultaat: De nieuwe waarde voor sleutel "001" wordt correct opgeslagen en opgehaald, terwijl de grootte
...
- **Toelichting en Onderbouwing:**
    - De `RemcoBST` is een custom implementatie van een Binary Search Tree (BST), specifiek ontworpen voor het efficiënt
      doorzoeken van stations op basis van hun code. De structuur staat bekend om de snelle zoekoperaties die het
...
#### **Testen van RemcoBST:** [RemcoBSTTest](test/datastructures/RemcoBSTTest.java)

1. **Test Vinden op Basis van Code (`testFindByCode`):**
...
2. **Test Niet Gevonden op Basis van Code (`testNotFoundByCode`):**
    - Test het scenario waar de gezochte code niet aanwezig is in de BST.
    - Verwacht resultaat: Retourneert `null` voor een niet-bestaande code "999".

3. **Test Structuur van de BST (`toGraphViz`):**
    - De `toGraphViz` methode kan worden gebruikt om de structuur van de BST te visualiseren en te verifiëren dat de
      boom correct is opgebouwd. Dit is vooral handig voor debugging en het visualiseren van de boomstructuur.
...
4. **Test Verwijderen uit Lege Heap (`testRemoveFromEmptyHeap`):**
    - Controleert of het verwijderen van een element uit een lege heap een correcte uitzondering gooit.
...
- **Toelichting en Onderbouwing:**
    - De `Graph` class is het kernonderdeel van de applicatie, dienend als de basis voor het spoorwegnetwerk. Deze
      implementatie is gekozen vanwege de flexibiliteit en efficiëntie die een graafstructuur biedt voor het
      representeren van complexe netwerken zoals spoorwegen.
- **Beschrijving van de Functionaliteit:**
    ...
    - De `toGraphViz` methode genereert een representatie van de graaf in GraphViz formaat, wat nuttig is voor
      visualisatie en debugging doeleinden.
...
1. **Test Toevoegen en Ophalen van Stations (`testAddAndGetStations`):**
    ...
    - Verwacht resultaat: Bevestigt de aanwezigheid van twee toegevoegde stations en de correcte grootte van de
      stationscollectie.
...
3. **Test GraphViz Formaat (`testToGraphVizFormat`):**
    ...
    - Verwacht resultaat: De GraphViz string bevat de verwachte verbinding tussen stations.
...
- **Beschrijving van de Functionaliteit:**
    - Het algoritme gebruikt een `Graph` object en een speciale MinHeap (`DijkstraMinHeap`) om de kortste afstanden van
      een startstation naar alle andere stations in de graaf te berekenen. Het houdt de kortste afstanden en de vorige
      stations bij in een map voor verdere routeconstructie.
...
- **Opmerking over Testen:**
    - Hoewel Dijkstra's Algoritme zelf niet direct is getest vanwege tijdbeperkingen en het feit dat de vereiste 75%
      code coverage al was bereikt, is de essentiële component `DijkstraMinHeap` wel grondig getest. Deze tests bieden
      indirecte zekerheid over de functionaliteit van het Dijkstra's Algoritme.

2. **Test Print Route Planning Station naar Station met
   Dijkstra (`testPrintRoutePlanningStationToStationDijkstra`):**
    - Test of de routeplanning met Dijkstra's algoritme correct werkt.
    - Verwacht resultaat: Een pad wordt correct geïdentificeerd tussen de opgegeven stations.

### Testen van DijkstraMinHeap

1. **Test Toevoegen en Verwijderen (`addAndPopTest`):**
...
3. **Test Verwijderen uit Lege Heap (`popFromEmptyHeapTest`):**
    - Test het gedrag van de heap bij het verwijderen uit een lege heap.
    - Verwacht resultaat: Het aanroepen van `pop` op een lege heap gooit een `IllegalStateException`.

4. **Test Afstandsvermindering voor Niet-Bestaand Station (`decreaseKeyNonExistingStationTest`):**
...
- **Beschrijving van de Functionaliteit:**
    - Het algoritme gebruikt een prioriteitswachtrij (`PriorityQueue`) en een heuristische functie om de meest
      ...
      evenals een lijst van stations waarvan elk station is afgeleid. Deze datastructuur wordt gebruikt om het meest
      efficiënte pad te herleiden.
...
1. **Test Uitvoering van het Algoritme (`testAStarAlgorithm`):**
    - Deze test controleert of het algoritme correct het meest efficiënte pad vindt tussen gegeven start- en
      eindstations.
    - Verwacht resultaat: Het pad bestaat uit de stations in de volgorde: Start Station → Intermediate Station → End
      Station.
...
- **Toelichting en Onderbouwing:**
    - Prim's Algoritme is geïmplementeerd om de Minimum Cost Spanning Tree (MCST) te berekenen binnen het
      ...
      stations verbindt met de minimale totale lengte, zonder cycli te vormen.
- **Beschrijving van de Functionaliteit:**
...
- **Opmerking over Testen:**
    - De functionaliteit van het Prim's Algoritme is indirect getest via de `AdvancedAlgorithmsCommand` class. Deze
      tests zijn essentieel om te verifiëren dat het algoritme correct werkt binnen de context van de applicatie.

1. **Test Print Stations in Rechthoek (`testPrintStationsInRectangle`):**
...
3. **Test Print Route Planning Station naar Station met A* (`testPrintRoutePlanningStationToStationAStar`):**
    - Verifieert of de routeplanning met het A* algoritme correct functioneert.
    - Verwacht resultaat: Een efficiënt pad wordt gevonden tussen de opgegeven stations.
...
- **Beschrijving van de Functionaliteit:**
    - De class bevat methoden voor het inladen van stations en tracks uit CSV-bestanden. Reguliere expressies worden
      ...
      worden overgeslagen. Dit zorgt voor nauwkeurige en betrouwbare data-invoer in de applicatie.
...
2. **Test Laden naar RemcoList (`testRemcoListNotEmpty` en `testValidRemcoListStationData`):**
    - Test of stations correct worden geladen in een `RemcoList` met behulp van dezelfde reguliere expressies.
    - Verwacht resultaat: De `RemcoList` met stations is niet leeg en bevat correct geparseerde data.
...
3. **Test Laden van Tracks (`testValidTrackData`):**
    - Controleert of de methode correct tracks laadt met reguliere expressies, waarbij alleen geldige data wordt
      geaccepteerd.
    - Verwacht resultaat: Tracks lijst is niet leeg en bevat correct geparseerde data voor elke track.
